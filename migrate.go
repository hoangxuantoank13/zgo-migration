...
import (
	...
	iurl "github.com/zgo-migration/migrate/v4/internal/url"
...
func (m *Migrate) read(direction source.Direction, ret chan<- interface{}) {
	defer close(ret)

	identifierSlice, err := m.sourceDrv.GetAllSource()
	if err != nil {
		ret <- err
		return
	}

	for _, identifier := range identifierSlice {
		if m.stop() {
			return
		}

		if ok, err := m.databaseDrv.IsMigrationExist(identifier, direction); err == nil && !ok {
			migr, err := m.newMigration(identifier, direction)
			if err != nil {
				ret <- err
				return
			}
...
func (m *Migrate) runMigrations(ret <-chan interface{}) error {
	for r := range ret {
		...
		case *Migration:
			...
			if err := m.databaseDrv.StoreMigration(migr.Raw, migr.Identifier, migr.Direction); err != nil {
				return err
			}
...
func (m *Migrate) newMigration(identifier string, direction source.Direction) (*Migration, error) {
	var migr *Migration

	if direction == source.Up {
		r, raw, err := m.sourceDrv.ReadUp(identifier)
		if os.IsNotExist(err) {
			// create "empty" migration
			migr, err = NewMigration(nil, "", direction, "")
			if err != nil {
				return nil, err
			}
			...
			migr, err = NewMigration(r, identifier, direction, raw)
			if err != nil {
				return nil, err
			}
		}
		...
		r, raw, err := m.sourceDrv.ReadDown(identifier)
		if os.IsNotExist(err) {
			// create "empty" migration
			migr, err = NewMigration(nil, "", direction, "")
			if err != nil {
